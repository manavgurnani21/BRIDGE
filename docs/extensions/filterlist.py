...
GROUP_ORDER = [
    "Benchmark across six cell lines",
    "Motif discovery",
    "Variant-aware prediction",
]
...
class CardDirective(SphinxDirective):
    ...
    def get_notebook_title(self, path):
        ...
        # If you cannot find it in the tutorials, then look in the user guide.
        if not os.path.exists(notebook_path):
            user_guide_dir = os.path.join(docs_root, "user_guide/")
            notebook_path2 = os.path.join(user_guide_dir, path)
            if not notebook_path2.endswith(".ipynb"):
                notebook_path2 += ".ipynb"

            if os.path.exists(notebook_path2):
    ...
    def get_index_header(self):
        ...
        """
        # Get the title from the model group index's top level header
